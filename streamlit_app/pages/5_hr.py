...
page_header("👥 人事管理", "员工能力画像、工作指标与贡献排行榜")
...
with st.expander("➕ 添加员工", expanded=False):
    with st.form("add_emp"):
        ...
        emp_id = c2.text_input("工号")
        c3, c4 = st.columns(2)
        dept = c3.text_input("部门")
        pos = c4.text_input("职位")
        feishu_id = st.text_input("飞书Open ID（选填）")
        skills_text = st.text_input("技能标签（逗号分隔）", placeholder="Python, 项目管理, 数据分析")
